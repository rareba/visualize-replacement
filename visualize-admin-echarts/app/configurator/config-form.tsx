  ...
  const handleChange = useEvent(async (e: SelectChangeEvent<unknown>) => {
    if (e.target.value !== FIELD_VALUE_NONE) {
      ...
      if (isColorInConfig(chartConfig)) {
        // Skip sequential/diverging color types (heatmap) as they use different palette handling
        const colorType = chartConfig.fields.color.type;
        if (colorType === "sequential" || colorType === "diverging") {
          return;
        }
        const palette =
