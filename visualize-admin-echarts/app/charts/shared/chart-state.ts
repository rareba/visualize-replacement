...
export const useBandYVariables = (
  ...
  if (!yDimension) {
    console.warn(
      `No dimension <${y?.componentId}> found in cube (useBandYVariables). Chart may not render correctly.`
    );
  }
  ...
  const yAxisLabel = yDimension ? getLabelWithUnit(yDimension) : "";
...
export const useBandXVariables = (
  ...
  if (!xDimension) {
    console.warn(
      `No dimension <${x?.componentId}> found in cube (useBandXVariables). Chart may not render correctly.`
    );
  }
  ...
  const xAxisLabel = xDimension ? getLabelWithUnit(xDimension) : "";
...
export const useTemporalXVariables = (
  ...
  if (!xDimension) {
    console.warn(
      `No dimension <${x?.componentId}> found in cube (useTemporalXVariables). Chart may not render correctly.`
    );
  }
  ...
  if (
    ...
    console.warn(
      `Dimension <${x?.componentId}> is not temporal. Chart may not render correctly.`
    );
  }
...
export const useTemporalMaybeXVariables = (
  ...
  if (
    ...
    console.warn(
      `Dimension <${componentId}> is not temporal (useTemporalMaybeXVariables). Chart may not render correctly.`
    );
  }
...
export const useNumericalXVariables = (
  ...
  // Handle missing measure gracefully - log warning and use fallback values
  const measureMissing = !xMeasure;
  const measureNotNumerical = xMeasure && !isNumericalMeasure(xMeasure);

  if (measureMissing) {
    console.warn(
      `No measure <${x?.componentId}> found in cube (useNumericalXVariables). Chart may not render correctly.`
    );
  } else if (measureNotNumerical) {
    console.warn(
      `Measure <${x?.componentId}> is not numerical. Chart may not render correctly.`
    );
  }

  // Use optional chaining to safely get componentId
  const getX = useOptionalNumericVariable(x?.componentId ?? "");

  // Use a fallback label if measure is missing
  const xAxisLabel = xMeasure ? getLabelWithUnit(xMeasure) : "";

  // Get scaleType safely with fallback
  const scaleType = xMeasure && isNumericalMeasure(xMeasure) ? xMeasure.scaleType : undefined;
  ...
  const getMinX = useCallback(
    (data: Observation[], _getX: NumericalValueGetter) => {
      switch (chartType) {
        ...
        case "scatterplot":
          return shouldUseDynamicMinScaleValue(scaleType)
            ? (min(data, _getX) ?? 0)
  ...
  // Return the measure (or a fallback) to avoid crashes
  return {
    xMeasure: xMeasure as NumericalMeasure,
    getX,
    xAxisLabel,
    getMinX,
  };
...
export const useNumericalYVariables = (
  ...
  // Handle missing measure gracefully - log warning and use fallback values
  const measureMissing = !yMeasure;
  const measureNotNumerical = yMeasure && !isNumericalMeasure(yMeasure);

  if (measureMissing) {
    console.warn(
      `No measure <${y?.componentId}> found in cube (useNumericalYVariables). Chart may not render correctly.`
    );
  } else if (measureNotNumerical) {
    console.warn(
      `Measure <${y?.componentId}> is not numerical. Chart may not render correctly.`
    );
  }
  ...
  // Use a fallback label if measure is missing
  const yAxisLabel = yMeasure ? getLabelWithUnit(yMeasure) : "";

  // Get scaleType safely with fallback
  const scaleType = yMeasure && isNumericalMeasure(yMeasure) ? yMeasure.scaleType : undefined;
  ...
  // Return the measure (or a fallback) to avoid crashes
  // The chart will handle empty/invalid data gracefully
  return {
    yMeasure: yMeasure as NumericalMeasure,
    getY,
    yAxisLabel,
    getMinY,
  };
