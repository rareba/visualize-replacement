...
// Donut Config (variant of Pie with inner radius)
const DonutFields = t.intersection([
  t.type({
    y: t.intersection([GenericField, ShowValuesFieldExtension]),
    segment: PieSegmentField,
    color: SegmentColorField,
  ...
  t.partial({
    animation: AnimationField,
    innerRadius: t.number, // 0-1, percentage of outer radius
  }),
]);
export type DonutFields = t.TypeOf<typeof DonutFields>;

const DonutConfig = t.intersection([
  GenericChartConfig,
  t.type(
    {
      chartType: t.literal("donut"),
      fields: DonutFields,
    },
    "DonutConfig"
  ),
...
const HeatmapFields = t.type({
  x: t.intersection([GenericField, SortingField]), // dimension for x-axis
  y: t.intersection([GenericField, SortingField]), // dimension for y-axis
  value: GenericField, // measure for cell color intensity
  color: t.union([
    t.type({
...
export type HeatmapFields = t.TypeOf<typeof HeatmapFields>;

const HeatmapConfig = t.intersection([
  GenericChartConfig,
  t.type(
    {
      chartType: t.literal("heatmap"),
      fields: HeatmapFields,
    },
    "HeatmapConfig"
  ),
]);
export type HeatmapConfig = t.TypeOf<typeof HeatmapConfig>;

// Boxplot Config (statistical distribution)
const BoxplotFields = t.type({
  x: t.intersection([GenericField, SortingField]), // category dimension
  y: GenericField, // measure for boxplot values
  color: t.union([SegmentColorField, SingleColorField]),
});
...
const WaterfallFields = t.type({
  x: t.intersection([GenericField, SortingField]), // category dimension
  y: t.intersection([GenericField, ShowValuesFieldExtension]), // measure
  color: SingleColorField,
});
export type WaterfallFields = t.TypeOf<typeof WaterfallFields>;

const WaterfallConfig = t.intersection([
  GenericChartConfig,
  t.type(
    {
      chartType: t.literal("waterfall"),
      fields: WaterfallFields,
    },
    "WaterfallConfig"
  ),
]);
...
export const isDonutConfig = (
  chartConfig: ChartConfig
): chartConfig is DonutConfig => {
  return chartConfig.chartType === "donut";
};

export const isRadarConfig = (
  chartConfig: ChartConfig
): chartConfig is RadarConfig => {
...
export const isHeatmapConfig = (
  ...
  return chartConfig.chartType === "heatmap";
};
