...
const useTableState = (
  ...
  const tableColumns = useMemo(() => {
    ...
    const columns = orderedTableColumns.map((c) => {
      ...
      if (!headerComponent) {
        console.warn(
          `No dimension <${c.componentId}> found in cube (tableColumns). Column may not render correctly.`
        );
        // Return a minimal column definition to prevent crash
        return {
          Header: c.componentId,
          id: c.componentId,
          accessor: (d: Observation) => d[c.componentId],
